...
import torch_xla.distributed.xla_nn as xnn

def visualize2DSoftmax(X, y, model, device):
    ...
    device -- the compute device to perform visualization on (CPU or TPU/GPU)
    """
    ...
    with torch.no_grad():
        preds = model(torch.tensor(xy_v, dtype=torch.float32).to(device)) # Move input to device
        preds = F.softmax(preds, dim=1).cpu().numpy() # Move predictions back to CPU for numpy
...
def run_epoch(model, optimizer, data_loader, loss_func, device, results, score_funcs, prefix="", desc=None):
    ...
    para_loader = pl.ParallelLoader(data_loader, [device]) # Parallel Loader for TPU
    train_loader_loop = para_loader.per_device_loader(device) # Get loader for current device

    for inputs, labels in tqdm(train_loader_loop, desc=desc, leave=False): # Iterate through parallel loader
        ...
        if model.training:
            optimizer.zero_grad() # Optimizer.zero_grad() should be before loss.backward() in XLA
            loss.backward()
            xm.optimizer_step(optimizer) # Use xm.optimizer_step for TPU optimizer step
        ...
        if len(score_funcs) > 0 and isinstance(labels, torch.Tensor):
            ...
            labels = labels.cpu().detach().numpy() # Move labels back to CPU for numpy
            y_hat = y_hat.cpu().detach().numpy() # Move predictions back to CPU for numpy
...
def train_simple_network(model, loss_func, train_loader, test_loader=None, score_funcs=None, 
                         epochs=50, device=None, checkpoint_file=None, lr=0.001):
    ...
    device -- the compute lodation to perform training, defaults to TPU if available, then GPU, then CPU
    ...
    #Place the model on the correct compute resource (CPU or TPU)
...
def moveTo(obj, device): # Keep moveTo for general utility, though not strictly needed for TPU loaders
    """
...
def train_network(model, loss_func, train_loader, val_loader=None, test_loader=None,score_funcs=None, 
                         epochs=50, device=None, checkpoint_file=None, 
...
class EmbeddingPackable(nn.Module):
    ...
    def forward(self, input):
        if type(input) == torch.nn.utils.rnn.PackedSequence:
            ...
            sequences, lengths = torch.nn.utils.rnn.pad_packed_sequence(input.cpu(), batch_first=True) # Unpack on CPU for pack_padded_sequence compat
            #Embed it
            sequences = self.embd_layer(sequences.to(input.data.device)) # Move to device before embedding
            #And pack it into a new sequence
            return torch.nn.utils.rnn.pack_padded_sequence(sequences, lengths.cpu(), # Pack lengths on CPU
                                                           batch_first=True, enforce_sorted=False).to(input.data.device) # Move packed sequence to device
